...
import { Briefcase, MapPin, Clock, Calendar, Loader2, CheckCircle2 } from "lucide-react";
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-xl bg-light-bg p-8 text-center">
        ...
        <button onClick={onDone} className="mt-2 text-sm font-semibold text-accent hover:underline">
          إغلاق
        </button>
      </div>
    );
    ...
    <form onSubmit={onSubmit} className="space-y-4 rounded-xl bg-light-bg p-6">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <input required placeholder="رقم الجوال" value={form.phone} onChange={set("phone")}
          className="w-full rounded-lg border border-border bg-white px-4 py-2.5 text-right outline-none focus:border-accent" />
        <input placeholder="المؤهل العلمي" value={form.education} onChange={set("education")}
          className="w-full rounded-lg border border-border bg-white px-4 py-2.5 text-right outline-none focus:border-accent" />
        <input placeholder="سنوات الخبرة" value={form.experience_years} onChange={set("experience_years")}
          className="w-full rounded-lg border border-border bg-white px-4 py-2.5 text-right outline-none focus:border-accent" />
        <input placeholder="رابط السيرة الذاتية (Google Drive مثلاً)" value={form.cv_url} onChange={set("cv_url")}
          className="w-full rounded-lg border border-border bg-white px-4 py-2.5 text-right outline-none focus:border-accent" />
      </div>
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={submitting}
          className="rounded-lg bg-gradient-brand px-6 py-2.5 text-sm font-bold text-white shadow-md disabled:opacity-60">
          {submitting ? "جاري الإرسال..." : "إرسال الطلب"}
        </button>
        <button type="button" onClick={onDone} className="text-sm font-semibold text-muted-foreground hover:underline">
          إلغاء
        </button>
      </div>
    </form>
            ...
            data.map((j, i) => (
              ...
              >
                <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
                      ...
                      <div className="mt-2 flex flex-wrap gap-4 text-xs text-muted-foreground">
                        ...
                        {j.deadline && (
                          <span className="flex items-center gap-1"><Calendar className="h-3.5 w-3.5" />
                            آخر موعد: {new Date(j.deadline).toLocaleDateString("ar-SA-u-nu-latn")}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <button
                    onClick={() => setOpenJobId((id) => (id === j.id ? null : j.id))}
